...
function MagiView({ name, id, chosenMagi, onMagiEditor, onHover }: MagiViewProps) {
  ...
  return (
    <div className="magiCard" onMouseEnter={() => onHover(name)} onMouseLeave={() => onHover('')}>
      ...
      {magi && (
        <Tooltip title="Energize rate" placement="bottom">
          <div className="energizeRate">+{magi.data.energize}</div>
        </Tooltip>
      )}
    </div>
  );
        ...
        return (
          <Tooltip placement="left" title={title}>
            <div>
              <Creature
                size={22}
                fillColor={startingCard ? "#3f7d20" : "#9a9a9a"}
              />
            </div>
          </Tooltip>
        );
          ...
          <Tooltip placement="left" title={title}>
            <div>
              <Relic
                size={22}
                fillColor={
                  startingCard ? "#3f7d20" : invalid ? "#990033" : "#9a9a9a"
                }
              />
            </div>
          </Tooltip>
...
export default function DeckView({
  ...
  const magiRegions = new Set<Region>([
    ...magi.map((card) => byName(card)?.region || REGION_ARDERIAL),
    REGION_UNIVERSAL,
  ]);

  // @ts-ignore
  const startingCards = new Set(
    [magiOne, magiTwo, magiThree]
      .map(
        (magi) => cards.find((card) => card?.name === magi)?.data.startingCards,
      )
      .flat(),
  );
  const deckCards = ourCards.slice(3);

  const distinctCards: string[] = deckCards.filter(
    (card, i) => deckCards.indexOf(card) === i,
  );
    ...
    <div>
      ...
      <div className="commands">
        <div style={{ lineHeight: "22px" }}>
          <Cards
            size={20}
            color={ourCards.length === 43 ? "#3f7d20" : "#990033"}
          />{" "}
          {ourCards.length}
        </div>
        <button onClick={onClearRegions}>
          Keep only the cards of Magi regions
        </button>
      </div>
      <div className="deckView">
        <ul>
          {distinctCards.map((card) => {
            ...
            const invalidCard =
              fullCard &&
              fullCard.type == TYPE_RELIC &&
              fullCard.region !== REGION_UNIVERSAL &&
              !magiRegions.has(fullCard.region);

            return (
              <li key={card}>
                <SimpleCardView
                  card={card}
                  className={cn("deckView", {
                ...
                <div className="cardActions">
                  <div className="cardCount">
                    [{deckCards.filter((c) => c === card).length}]
                  </div>
                  <div
                    className="cardRemove"
                    onClick={() => removeFromDeck(card)}
                  >
                    <Remove size={20} color={"#990033"} />
                  </div>
                  {deckCards.filter((c) => c === card).length <
                    MAX_COPIES_IN_DECK &&
                    ourCards.length < 43 && (
                      <div
                        className="cardAdd active"
                        onClick={() => addToDeck(card)}
                      >
                        <Add size={20} fillColor="#3f7d20" />
                      </div>
                    )}
                  {(deckCards.filter((c) => c === card).length ===
                    MAX_COPIES_IN_DECK ||
