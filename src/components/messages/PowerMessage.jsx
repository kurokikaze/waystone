  ...
  return card ? (
    <div className="BaseMessage">
      ...
      <div className="BaseMessage__message">
        <span className="PowerMessage__source">{card.card}</span> uses power{" "}
        <span className="PowerMessage__power">{power}</span>
      </div>
      <div onClick={onBreak} className="BaseMessage__close">
        &#10006;
      </div>
    </div>
  ) : null;
