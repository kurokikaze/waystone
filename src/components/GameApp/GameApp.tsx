...
import { ACTION_PASS, ACTION_PLAY } from "moonlands/dist/const";
...
function App({ engineConnector, onBreak, onReturnToBase }: AppProps) {
  ...
  const handleOurDiscardClose = useCallback(() => setDiscardShown(false), []);
  ...
  const currentPlayer: number = useSelector(
    (state: { activePlayer: number }) => state.activePlayer,
  );
  ...
  const cardsInOpponentDiscard: number = useSelector(
    getCardsCountInOpponentDiscard,
  );
    ...
    <div className="gameContainer">
      ...
      <div className="game">
        <DndProvider backend={HTML5Backend}>
          <>
            {message && message.type == MESSAGE_TYPE_POWER && (
              <EnhancedPowerMessage
                id={message.source}
                power={message.power}
                onBreak={onBreak}
              />
            )}
            {message && message.type == MESSAGE_TYPE_RELIC && (
              <RelicMessage card={message.card} player={message.player} />
            )}
            {message && message.type == MESSAGE_TYPE_SPELL && (
              <SpellMessage card={message.card} player={message.player} />
            )}
            {message && message.type == MESSAGE_TYPE_CREATURE && (
              <CreatureMessage card={message.card} player={message.player} />
            )}
            {message && message.type == MESSAGE_TYPE_PROMPT_RESOLUTION && (
              <PromptResolutionMessage
                card={message.chosenTarget}
                number={message.chosenNumber}
              />
            )}
            ...
            <div className="middleZones">
              <div className="zone-placeholder">
                ...
                <div
                  className="discardCounter"
                  onClick={handleOpponentDiscardClick}
                >
                  {cardsInOpponentDiscard}
                </div>
              </div>
              <ZoneOpponentActiveMagi
                zoneId="opponentActiveMagi"
                name="Opponent Active Magi"
                engineConnector={engineConnector}
              />
              <ZonePlayerRelics
                zoneId="opponentRelics"
                name="Opponent Relics"
                engineConnector={engineConnector}
              />
            </div>
            <ZoneOpponentInPlay
              zoneId="opponentInPlay"
              name="Opponent in play"
              engineConnector={engineConnector}
            />
            <ZonePlayerInPlay
              zoneId="playerInPlay"
            ...
            <div className="middleZones">
              <ZonePlayerRelics
                ...
                engineConnector={engineConnector}
              />
              <ZonePlayerActiveMagi
                zoneId="playerActiveMagi"
                name="Player Active Magi"
                engineConnector={engineConnector}
              />
              <div className="zone-placeholder">
                <div className="discardCounter" onClick={handleOurDiscardClick}>
                  {cardsInOurDiscard}
                </div>
            ...
            <ActionCardView />
            {ourTurn &&
              currentStep !== STEP_ENERGIZE &&
              currentStep !== STEP_DRAW && (
                <>
                  <button onClick={onPass}>
                    {currentStep === null ? "Start the game" : "Pass"}
                  </button>
                </>
              )}
            ...
            {discardShown && (
              <div className="discardOverlay">
                <h2>Discard</h2>
                <div className="closeIcon" onClick={handleOurDiscardClose}>
                  &times;
                </div>
                <ZoneDiscard zoneId="playerDiscard" name="Player discard" />
              </div>
            )}
            {opponentDiscardShown && (
              <div className="discardOverlay">
                <h2>Opponent&apos;s Discard</h2>
                <div className="closeIcon" onClick={handleOpponentDiscardClose}>
                  &times;
                </div>
