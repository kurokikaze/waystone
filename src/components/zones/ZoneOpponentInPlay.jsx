...
import { ANIMATION_CREATURE_DISCARDED, STEP_ATTACK } from "../../const";
import {
  getCardDetails,
...
function ZoneOpponentInPlay({ name, engineConnector }) {
  ...
  const isOnUnfilteredPrompt =
    isOnCreaturePrompt && UNFILTERED_CREATURE_PROMPTS.includes(promptType);
  const isOnFilteredPrompt =
    isOnCreaturePrompt && FILTERED_CREATURE_PROMPTS.includes(promptType);
  const animation = useSelector(getAnimation);
  const defeatedId =
    animation && animation.type === ANIMATION_CREATURE_DISCARDED
      ? animation.target
      : null;
  ...
  const cardClickHandler = isOnCreaturePrompt
    ? (cardId) => {
  ...
  return (
    <div
      className={cn("zone", "zone-creatures", { "zone-active": active })}
      data-zone-name={name}
