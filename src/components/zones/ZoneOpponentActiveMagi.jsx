...
import { ANIMATION_MAGI_DEFEATED, STEP_ATTACK } from "../../const";
...
const isOnFilteredMagiPrompt = (state) => {
  const isOnMWCPrompt =
    state.prompt && state.promptType === PROMPT_TYPE_MAGI_WITHOUT_CREATURES;
  return (
    isOnMWCPrompt &&
    !getCardDetails(state).inPlay.some(
      (card) => card.data.controller !== 1 && card.card.type === TYPE_CREATURE,
    )
  );
};
...
function ZoneOpponentActiveMagi({ name, zoneId, engineConnector }) {
  ...
  const guarded = inPlayContent.inPlay.some(
    (card) => card.data.controller !== 1 && card.card.type === TYPE_CREATURE,
  );
  ...
  const defeatedId =
    animationData && animationData.type === ANIMATION_MAGI_DEFEATED
