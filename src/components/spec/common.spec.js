...
describe("cardDataTransformer", () => {
  it("Static abilities - Double Strike", () => {
    ...
    expect(resultingCard.card.data.attacksPerTurn).toEqual(
      1,
      "Arbolls original data shows 1 attack per turn",
    );
    expect(resultingCard.modifiedData.attacksPerTurn).toEqual(
      2,
      "Arboll now has 2 attacks per turn, modified by Yaki static ability",
    );
  });
  ...
  it("Static abilities - Invigorate", () => {
    ...
    expect(resultingCard.modifiedData.energize).toEqual(
      6,
      "Grega energize rate modified by Water of Life is 6",
    );
    expect(resultingCard.card.data.energize).toEqual(
      5,
      "Grega original energize rate is still 5",
    );
  });
  ...
  it("Static abilities - none", () => {
    ...
    expect(resultingCard.modifiedData.attacksPerTurn).toEqual(
      1,
      "Arboll has 1 attack per turn,  not modified by anything",
    );
  });
  ...
  it("Static abilities - none (Stagadan)", () => {
    ...
    expect(transformedCard.modifiedData.attacksPerTurn).toEqual(
      1,
      "Stagadan has 1 attack per turn,  not modified by anything",
    );
  });
});
