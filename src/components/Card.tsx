...
import { attackAnimation } from "./gsapAnimations.js";
...
function Card({
  ...
  useLayoutEffect(attackAnimation, [attacker, attackNumber]);

  const energyChange = useSelector((state: State) =>
    state.energyLosses.filter((loss) => loss.card === id),
  );
  const energyEffectsShown = useSelector(
    (state: State) => state.energyAnimationsShown,
  );
  ...
  const prevPos: [number, number] | undefined = useSelector(
    (state: State) => state.lastPositions[id],
  );
  useLayoutEffect(() => {
    // Appearance
    if (ref.current && prevPos instanceof Array) {
      ...
      tl.from(ref.current, {
        x: prevPos[0] - myBox.left,
        y: prevPos[1] - myBox.top,
        duration: 0.2,
      });
      return () => {
        tl.clear();
  ...
  useLayoutEffect(() => {
    const tlsToClear: gsap.core.Timeline[] = []

    setTimeout(() => {
      if (ref.current) {
        // let cleanupNeeded = false;
        (ref.current as HTMLElement)
          .querySelectorAll(".energyDamage")
          .forEach((node) => {
            let tl = gsap.timeline()

            const val = parseInt(node.innerHTML, 10);
            tl.to(node, {
              y: -100,
              duration: 0.8,
              content: -val,
            ...
            // if (
            //   !node.classList.contains("fadeAway") &&
            //   !energyEffectsShown.has(id)
            // ) {
        ...
        // if (cleanupNeeded) {
          ...
          //   if (ref.current) {
          //     (ref.current as HTMLElement)
          //       .querySelectorAll(".energyGain.fadeAway")
          //       .forEach((node) => node.classList.add("fadedFully"));
          //     (ref.current as HTMLElement)
          //       .querySelectorAll(".energyDamage.fadeAway")
          //       .forEach((node) => node.classList.add("fadedFully"));
          //   }
    ...
    return () => {
      tlsToClear.forEach(tl => tl.clear())
    }
  }, [energyChange]);
  ...
  const regionStyle =
    cardStyle != CARD_STYLE_LOCKET && card && "region" in card && card.region
      ? regionClass[card.region as Region]
      : null;
  ...
  const classes = cn(
    ...
    card ? typeClass[card.type as CardType] : null,
    {
      ...
      canPackHunt:
        card && modifiedData
          ? card.data.canPackHunt &&
          data.attacked < (modifiedData.attacksPerTurn || 0) &&
          !pack
          : null,
    },
    className,
      ...
      <div
        ...
        style={
          cardStyle == CARD_STYLE_DRAGGABLE
            ? { backgroundImage: `url(${getCardUrl(card, false)})` }
            : {}
        }
      >
        {cardStyle != CARD_STYLE_DRAGGABLE && (
          <img
            src={getCardUrl(card, cardStyle == CARD_STYLE_LOCKET)}
            alt={card ? card.name : ""}
          />
        )}
        {data && (
          <>
            {card && data.energy && data.energy !== card.cost ? (
              <div className="startingEnergy">{card.cost}</div>
            ) : null}
            <div className="cardEnergy">{data.energy || ""}</div>
          </>
        )}
      </div>
      {damage
        .filter(({ id }) => !energyEffectsShown.has(id))
        .map(({ id, value }) => (
          <div key={id} data-effect-id={id} className="energyDamage">
            {value}
          </div>
        ))}
      {energyGain
        .filter(({ id }) => !energyEffectsShown.has(id))
        .map(({ id, value }) => (
          <div key={id} data-effect-id={id} className="energyGain">
            {value}
          </div>
        ))}
    </div>
