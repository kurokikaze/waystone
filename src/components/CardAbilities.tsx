...
export const CardAbility = ({ name, cost, text, used, costTooHigh, onClick }: CardAbilityProps) =>
(
	<div className={cn('ability', { 'used': used, 'costTooHigh': costTooHigh })}>
		<span className='abilityName' onClick={onClick as MouseEventHandler<HTMLSpanElement>}>{name}</span>&nbsp;&mdash;&nbsp;<span className='abilityCost'>{cost}</span>: <span>{text}</span>
	</div>
);
...
export const OpponentCardAbility = ({ name, cost, text }: OpponentCardAbilityProps) =>
(
