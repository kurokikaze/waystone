...
function ActionCardView() {
	...
	return <div>
		{show && <div className={cn('action-card-view', { 'power-view': power, 'relic-view': relic, 'spell-view': spell })}>
			{power && <div className='power-view'>
				<h3>{power.name}</h3>
				<p>{power.text}</p>
			</div>}
			{relic && <div className="cardViewHolder relic"><div className="cardView"><img src={`/cards/${camelCase(relicCard)}.jpg`}/></div></div>}
			{spell && <div className="cardViewHolder spell"><div className="cardView"><img src={`/cards/${camelCase(spellCard)}.jpg`}/></div></div>}
		</div>}
	</div>;
}
