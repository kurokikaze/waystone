...
function DeckContent({ deck }) {
  ...
  return (
    <div className="deckGrid">
      ...
      <Row>
        <Col span={8}>
          <img className="smaller" src={`/cards/${camelCase(magiOne)}.jpg`} />
        </Col>
        <Col span={8}>
          <img className="smaller" src={`/cards/${camelCase(magiTwo)}.jpg`} />
        </Col>
        <Col span={8}>
          <img className="smaller" src={`/cards/${camelCase(magiThree)}.jpg`} />
        </Col>
      </Row>
      <Row>
        <Col span={24}>
      ...
      <Row>
        <Col span={24} className="cardList">
          {Object.entries(cardList).map(([card, num], i) => (
            <div key={i} className="cardLink">
              {num}x&nbsp;{<CardView name={card} />}
            </div>
          ))}
        </Col>
      </Row>
