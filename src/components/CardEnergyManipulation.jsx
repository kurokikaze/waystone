...
export const SimpleButton = ({ name, disabled, style, onClick }) => (
  <div
    className={cn("simpleButton", { disabled: disabled })}
    style={style}
    onClick={onClick}
  >
    {name}
...
export const withEnergyManipulation =
  (Component) =>
  ({ id, data, ...props }) => {
    const dispatch = useDispatch();
    const freeEnergy = useSelector((state) => state.energyPrompt.freeEnergy);
    const currentEnergy =
      useSelector((state) => state.energyPrompt.cards[id]) || 0;
    ...
    const thisIsNotTheSource =
      restriction !== RESTRICTION_EXCEPT_SOURCE || id !== generatedBy;
    const card = byName(data.card.name);
    const ableToParticipate = !card || !card.data.energyStasis;
    return (
      <div>
        <Component id={id} data={data} {...props} />
        {thisIsNotTheSource && ableToParticipate && (
          <div className="energyManipulation">
            <SimpleButton
              name="-"
              style={{
                color: currentEnergy === 0 ? "#ccc" : "#000",
                cursor: "pointer",
              }}
              onClick={handleMinusEnergy}
            ...
            <SimpleButton
              ...
              onClick={handlePlusEnergy}
              disabled={freeEnergy === 0}
            />
          </div>
        )}
      </div>
