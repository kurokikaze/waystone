...
function PromptChooseCards({ engineConnector }) {
  ...
  const triggerElement = (cardName) =>
    availableCards.includes(cardName) &&
  ...
  return (
    <div className="promptWindow promptChooseCards">
      <div className="promptMagi">
        <Card id={magi.id} card={{ name: magi.card }} data={{}} />
      </div>
      <h1>Choose starting cards</h1>
      <div className="cardsRow">
        {cards.map((card) => (
          <div
            className={cn("cardSelect", {
              chosen: selected.includes(card),
              notAvailable: !availableCards.includes(card),
            })}
            key={card}
          >
            <Card
