  ...
  return (
    <div className="promptOverlay" ref={overlay}>
      ...
      {promptType === PROMPT_TYPE_CHOOSE_CARDS && (
        <PromptChooseCards engineConnector={engineConnector} />
      )}
      {promptType === PROMPT_TYPE_NUMBER && (
        <PromptChooseNumber engineConnector={engineConnector} />
      )}
      {promptType === PROMPT_TYPE_MAY_ABILITY && (
        <PromptMayEffect engineConnector={engineConnector} />
      )}
      {promptType === PROMPT_TYPE_CHOOSE_N_CARDS_FROM_ZONE && (
        <PromptChooseCardsInZone engineConnector={engineConnector} />
      )}
      {promptType === PROMPT_TYPE_CHOOSE_UP_TO_N_CARDS_FROM_ZONE && (
        <PromptChooseUpToNCardsInZone engineConnector={engineConnector} />
      )}
      {promptType === PROMPT_TYPE_REARRANGE_ENERGY_ON_CREATURES && (
        <PromptEnergyManipulation engineConnector={engineConnector} />
      )}
      {promptType === PROMPT_TYPE_DISTRIBUTE_ENERGY_ON_CREATURES && (
        <PromptEnergyDistribution engineConnector={engineConnector} />
      )}
      {promptType === PROMPT_TYPE_DISTRIBUTE_DAMAGE_ON_CREATURES && (
        <PromptDamageDistribution engineConnector={engineConnector} />
      )}
      {promptType === PROMPT_TYPE_REARRANGE_CARDS_OF_ZONE && (
        <PromptRearrangeCards engineConnector={engineConnector} />
      )}
      {promptType === PROMPT_TYPE_PLAYER && (
        <PromptChoosePlayer engineConnector={engineConnector} />
      )}
      {promptType === PROMPT_TYPE_POWER_ON_MAGI && (
        <PromptChoosePowerOnMagi engineConnector={engineConnector} />
      )}
      {promptType === PROMPT_TYPE_ALTERNATIVE && (
        <PromptAlternatives engineConnector={engineConnector} />
      )}
    </div>
  );
}
