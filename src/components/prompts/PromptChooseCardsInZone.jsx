...
function PromptChooseCards({ engineConnector }) {
  ...
  const triggerElement = (cardName) =>
    setSelected((selected) =>
      selected.includes(cardName)
        ? selected.filter((e) => e !== cardName)
        : [...selected, cardName],
    );

  return (
    <div
      className={cn("promptWindow promptChooseCards", {
        promptContracted: contracted,
      })}
    >
      <h1>Choose {numberOfCards} card(s)</h1>
      <div style={{ position: "absolute", top: 20, right: 20 }}>
        {contracted ? (
          <div onClick={() => setContracted(false)}>
            <Expand size={20} />
          </div>
        ) : (
          <div onClick={() => setContracted(true)}>
            <Contract size={20} />
          </div>
        )}
      </div>
      <div className="prompt-content">
        ...
        <div className="buttonHolder">
          <button
            onClick={handleSend}
            disabled={numberOfCards !== selected.length}
          >
            OK
          </button>
        </div>
