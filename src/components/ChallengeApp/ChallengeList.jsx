...
function ChallengeList({
  ...
  const hasChallenged = challenges.some(
    (challenge) => challenge.user === username,
  );
  return (
    <div className="challenges">
      {challenges.map((challenge) => (
        <div className="challenge" key={challenge.deckId || "test"}>
          ...
          <div>
            {challenge.user == username ? (
              ...
              <button
                onClick={() => acceptChallenge(challenge.user, currentDeck)}
              >
                Accept!
              </button>
            )}
      ...
      {!hasChallenged && (
        <div className="create_challenge">
          <button onClick={() => createChallenge(currentDeck)}>
            Create challenge!
          </button>
        </div>
      )}
