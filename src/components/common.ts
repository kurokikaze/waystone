...
export const cardMatchesSelector = (
  card: EnrichedCard,
  selector: SelectorTypeType,
  selectorParameter: any = null,
  ...
  switch (selector) {
    case SELECTOR_OWN_CREATURES: {
      return (
        ...
        card.data.controller === sourceController
      );
    }
    case SELECTOR_OWN_MAGI: {
      return (
        card.card.type === TYPE_MAGI &&
        card.data.controller === sourceController
      );
    }
    case SELECTOR_CREATURES_OF_PLAYER: {
      return (
        card.card.type === TYPE_CREATURE &&
        card.data.controller === selectorParameter
      );
    }
    case SELECTOR_ID: {
    ...
    case SELECTOR_OWN_CREATURES_WITH_STATUS: {
      switch (selectorParameter) {
        case STATUS_BURROWED:
          return (
            card.data.burrowed && card.data.controller === sourceController
          );
      }
      return false;
    }
...
type EnrichedCardWithModifiedData = EnrichedCard & { modifiedData: CardData };
export const transformCard =
  (staticAbilityCards: ExtendedCard[]) => (cardData: ConvertedCard) => {
    const card = cardData.card ? byName(cardData.card) : null;

    if (card) {
      const result: EnrichedCardWithModifiedData = {
        ...cardData,
        card,
        originalCard: card,
      ...
      staticAbilityCards.forEach((staticAbilityCard) => {
        staticAbilityCard.card.data.staticAbilities.forEach(
          (staticAbility: StaticAbilityType) => {
            if (
              cardMatchesSelector(
                result,
                staticAbility.selector,
                staticAbility.selectorParameter,
                staticAbilityCard.data.controller,
              )
            ) {
              ...
              switch (staticAbility.property) {
                case PROPERTY_POWER_COST: {
                  if (
                    "powers" in result.modifiedData &&
                    result.modifiedData.powers
                  ) {
                    result.modifiedData.powers = result.modifiedData.powers.map(
                      (power) => ({
                        ...power,
                        cost: modifierFunction(power.cost),
                      }),
                    );
                  }
                  break;
                ...
                case PROPERTY_ATTACKS_PER_TURN: {
                  result.modifiedData.attacksPerTurn = modifierFunction(
                    result.modifiedData.attacksPerTurn,
                  );
                  break;
                }
                case PROPERTY_ENERGIZE: {
                  result.modifiedData.energize = modifierFunction(
                    result.modifiedData.energize || 0,
                  );
                  break;
                }
              }
...
export const useCardData = (content: ConvertedCard[]) => {
  const staticAbilities =
    useSelector((state: State) => state.staticAbilities) || [];

  const transformer = transformCard(staticAbilities);
  return content.map(transformer);
};
...
export function mapCardDataFromProps(state: State, { id }: ConvertedCard) {
  ...
  // @ts-ignore
  const foundZone = Object.values(state.zones).find(
    (zone) => zone && zone.find(filter),
  );
  return {
    // @ts-ignore
...
export const useZoneContent = (zoneId: ZoneIdentifier) =>
  useSelector(getZoneContent(zoneId));

export const UNFILTERED_CREATURE_PROMPTS = [PROMPT_TYPE_SINGLE_CREATURE];
...
export const UNFILTERED_RELIC_PROMPTS = [PROMPT_TYPE_RELIC];

const getRestrictionFilter = (
  ...
  switch (restriction) {
    ...
    case RESTRICTION_CREATURE_TYPE:
      return (card) =>
        card.card.type === TYPE_CREATURE &&
        card.card.name.split(" ").includes(value);
    case RESTRICTION_ENERGY_EQUALS:
      return (card) =>
        card.card.type === TYPE_CREATURE && card.data.energy === value;
    case RESTRICTION_ENERGY_LESS_THAN:
      return (card) =>
        card.card.type === TYPE_CREATURE && card.data.energy < value;
    case RESTRICTION_ENERGY_LESS_THAN_STARTING:
      return (card) =>
        card.card.type === TYPE_CREATURE && card.data.energy < card.card.cost;
    case RESTRICTION_OWN_CREATURE:
      return (card) =>
        card.card.type === TYPE_CREATURE &&
        (card.data.controller || card.owner) === 1;
    ...
    case RESTRICTION_CREATURE_WAS_ATTACKED:
      return (card) =>
        card.card.type === TYPE_CREATURE && card.data.wasAttacked === true;
    case RESTRICTION_STATUS:
      switch (value) {
        case STATUS_BURROWED:
          return (card) => card.data.burrowed === true;
      }
  }
  return () => true;
};
...
export const getPromptFilter = (
  ...
  switch (promptType) {
    ...
    case PROMPT_TYPE_SINGLE_CREATURE_OR_MAGI:
      return (card) =>
        card.card.type === TYPE_MAGI || card.card.type === TYPE_CREATURE;
    ...
    case PROMPT_TYPE_SINGLE_CREATURE_FILTERED:
      if (promptParams) {
        if (promptParams.restrictions && promptParams.restrictions.length) {
          const checkers: SelectorFunction[] = promptParams.restrictions.map(
            ({ type, value }: RestrictionObjectType): SelectorFunction =>
              getRestrictionFilter(type, value),
          );
          return (card) =>
            checkers.map((checker) => checker(card)).every((a) => a === true); // combine checkers
        } else {
          return getRestrictionFilter(
            promptParams.restriction!,
            promptParams.restrictionValue,
          );
        }
      } else {
        return () => true;
    ...
    case PROMPT_TYPE_PAYMENT_SOURCE: {
      return (card) => {
        const promptParamsFixed = promptParams as PromptParamsFixed;
        const canPayAtAll =
          card.card.data?.paymentSource &&
          card.card.data?.paymentSource.includes(promptParamsFixed.paymentType);
        const haveEnoughEnergy =
          card.data.energy >= (promptParamsFixed?.paymentAmount || 0);
...
function getByProperty(
  ...
  switch (property) {
    ...
    case PROPERTY_POWER_COST:
      return (
        (target.card.data as CardData)?.powers?.find(
...
const gameStaticAbilities: EnhancedStaticAbilityType[] = [
  {
...
type CardsByZones = {
  inPlay: EnrichedCard[];
  playerActiveMagi: EnrichedCard[];
  opponentActiveMagi: EnrichedCard[];
};

type EnhancedStaticAbilityType = StaticAbilityType & {
  player: number;
...
export const getCardDetails = (state: State) => {
  ...
  const allZonesCards: CardsByZones = {
    inPlay: [...baseCards].map(
      (card: ConvertedCard): EnrichedCard => ({
        ...card,
        card: byName(card.card) as Card,
        originalCard: byName(card.card) as Card,
      }),
    ),
  ...
  const continuousStaticAbilities: EnhancedStaticAbilityType[] =
    state.continuousEffects
      .map((effect) =>
        effect.staticAbilities.map((ability) => ({
          ...ability,
          expiration: effect.expiration,
        })),
      )
      .flat();
  const zoneAbilities: EnhancedStaticAbilityType[] = [
    ...allZonesCards.inPlay,
    ...allZonesCards.playerActiveMagi,
    ...allZonesCards.opponentActiveMagi,
  ].reduce(
    (acc: EnhancedStaticAbilityType[], cardInPlay) =>
      cardInPlay && "card" in cardInPlay && cardInPlay.card.data.staticAbilities
        ? [
            ...acc,
            ...cardInPlay.card.data.staticAbilities.map(
              (a: StaticAbilityType): EnhancedStaticAbilityType => ({
                ...a,
                player: cardInPlay.data.controller,
              }),
            ),
          ]
        : acc,
  ...
  const staticAbilities = [
    ...gameStaticAbilities,
    ...zoneAbilities,
    ...continuousStaticAbilities,
  ].sort(
    (a, b) =>
      (propertyLayers[a.property] || 0) - (propertyLayers[b.property] || 0),
  );

  const resultingZones = staticAbilities.reduce(
    (oldState: CardsByZones, staticAbility): CardsByZones => {
      const {
        name,
        text,
        selector,
      ...
      for (let cardId in newState.inPlay) {
        ...
        if (
          ...
          newState.inPlay[cardId].data.affectedBy = [
            ...(newState.inPlay[cardId]?.data?.affectedBy || []),
            { name, text, expiration },
          ];

          const initialValue =
            property !== PROPERTY_POWER_COST
              ? getByProperty(currentCard, property, subProperty)
              : null;
          switch (property) {
            case PROPERTY_ENERGIZE: {
              const resultValue =
                ...
                operator === CALCULATION_SUBTRACT_TO_MINIMUM_OF_ONE
                  ? performCalculation(
                      operator,
                      initialValue,
                      typeof operandOne === "number" ? operandOne : 0,
                    )
                  : performCalculation(
            ...
            case PROPERTY_ATTACKS_PER_TURN: {
              const initialValue = getByProperty(
                currentCard,
                PROPERTY_ATTACKS_PER_TURN,
              );
              const { operator, operandOne } = staticAbility.modifier;
            ...
            case PROPERTY_ENERGY_LOSS_THRESHOLD: {
              ...
              newState.inPlay[cardId].card.data.energyLossThreshold =
                resultValue;
              break;
            }
            case PROPERTY_ABLE_TO_ATTACK: {
              ...
              const resultValue =
                operator === CALCULATION_SET ? operandOne : initialValue;
            ...
            case PROPERTY_POWER_COST: {
              if (currentCard.card.data.powers) {
                for (let powerId in currentCard.card.data.powers) {
                  ...
                  newState.inPlay[cardId].card.data.powers[powerId].cost =
                    resultValue;
                }
              }
              break;
