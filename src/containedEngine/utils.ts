...
const index = (
  ...
  else if (correctIs.length == 0) return obj.toString();
  else
    return index(
      obj[correctIs[0]] as unknown as MetaDataRecord,
      correctIs.slice(1).join("."),
      value,
    );
};
...
): ConvertedCard | HiddenConvertedCard => {
  if (!cardInGame.card) {
    return {
      id: cardInGame.id,
...
export const hideIfNecessary = (
  card: CardInGame,
  targetZone: ZoneType,
  isOpponent: boolean,
): CardInGame | HiddenCardInGame => {
...
export function convertServerCommand(
  initialAction: AnyEffectType,
  game: State,
  playerId: number,
  overrideHiding = false,
): ClientCommand | any | null {
  try {
  ...
  switch (action.type) {
    ...
    case ACTION_PLAY: {
      const cardPlayed =
        "card" in action
          ? game.getMetaValue(action.card, action.generatedBy)
          : action.payload.card; // ? action.payload.card : metaCard;
    ...
    case ACTION_ATTACK: {
      const attackSource: CardInGame = game.getMetaValue(
        action.source,
        action.generatedBy,
      );
      const attackTarget: CardInGame = game.getMetaValue(
        action.target,
        action.generatedBy,
      );
      ...
      if ("additionalAttackers" in action && action.additionalAttackers) {
        convertedAction.additionalAttackers = action.additionalAttackers.map(
          (card) => card.id,
        );
      }
      ...
      switch (action.promptType) {
        case PROMPT_TYPE_NUMBER: {
          return {
            ...
            min: parseInt(
              game.getMetaValue(action.min, action.generatedBy),
              10,
            ),
            max: parseInt(
              game.getMetaValue(action.max, action.generatedBy),
              10,
            ),
        ...
        case PROMPT_TYPE_CHOOSE_UP_TO_N_CARDS_FROM_ZONE: {
          ...
          const restrictions =
            ...
            (action.restriction
              ? [
                  {
                    type: game.getMetaValue(
                      action.restriction,
                      action.generatedBy,
                    ) as RestrictionType,
                    value: game.getMetaValue(
                      action.restrictionValue,
                      action.generatedBy,
                    ),
                  },
                ]
          ...
          const zoneOwner = game.getMetaValue(
            action.zoneOwner,
            action.generatedBy,
          );
          const numberOfCards = game.getMetaValue(
            action.numberOfCards,
            action.generatedBy,
          );
          const cardFilter = game.makeCardFilter(restrictions || []);
          const zoneContent = game.getZone(zone, zoneOwner).cards;
          const cards = restrictions
            ? zoneContent.filter(cardFilter)
            : zoneContent;
          const promptPlayer = game.getMetaValue(
            action.player,
            action.generatedBy,
          );
        ...
        case PROMPT_TYPE_REARRANGE_CARDS_OF_ZONE: {
          const zone = game.getMetaValue(
            action.promptParams.zone,
            action.generatedBy,
          );
          const zoneOwner = game.getMetaValue(
            action.promptParams.zoneOwner,
            action.generatedBy,
          );
          const numberOfCards = game.getMetaValue(
            action.promptParams.numberOfCards,
            action.generatedBy,
          );
          const zoneContent = game.getZone(zone, zoneOwner).cards;
        ...
        case PROMPT_TYPE_ANY_CREATURE_EXCEPT_SOURCE: {
          const actionSource = game.getMetaValue(
            action.source,
            action.generatedBy,
          );
        ...
        case PROMPT_TYPE_POWER_ON_MAGI: {
          const magi: CardInGame[] = game.getMetaValue(
            action?.magi,
            action.generatedBy,
          );
          return {
        ...
        case PROMPT_TYPE_ALTERNATIVE: {
          return {
            ...
            generatedBy: action.generatedBy,
            player: parseInt(
              game.getMetaValue(action.player, action.generatedBy),
              10,
            ),
          };
        }
    ...
    case ACTION_RESOLVE_PROMPT: {
      ...
      if ("target" in action && action.target) {
        promptAction.targetCard = convertCard(action.target) as
          | ConvertedCard
          | HiddenConvertedCard;
      ...
      if ("number" in action) {
        promptAction.number =
          (typeof action.number == "string"
            ? parseInt(action.number, 10)
            : action.number) || 0;
      }
      return promptAction;
    }
    ...
    case ACTION_EFFECT: {
      switch (action.effectType) {
        ...
        case EFFECT_TYPE_FORBID_ATTACK_TO_CREATURE: {
          ...
          const actionTarget = game.getMetaValue(
            action.target,
            action.generatedBy,
          );
          return {
        ...
        case EFFECT_TYPE_REARRANGE_ENERGY_ON_CREATURES: {
          const energyOnCreatures: Record<string, number> =
            game.getMetaValue(action.energyOnCreatures, action.generatedBy) ||
            {};
          return {
            type: action.type,
            effectType: action.effectType,
        ...
        case EFFECT_TYPE_ATTACK: {
          ...
          const additionalAttackers: CardInGame[] =
            game.getMetaValue(action.additionalAttackers, action.generatedBy) ||
            [];
          return {
            ...action,
            source: attackSource.id,
        ...
        case EFFECT_TYPE_DISCARD_CREATURE_FROM_PLAY: {
          const targetCard =
            typeof action.target == "string"
              ? game.getMetaValue<CardInGame[] | CardInGame>(
                  action.target,
                  action.generatedBy,
                )
              : action.target;
          const target =
            "length" in targetCard && targetCard.length
              ? targetCard[0]
              : targetCard;
          // @ts-ignore will be fixed in a future moonlands update
          const sourceCard: CardInGame | null =
            typeof action?.source == "string"
              ? // @ts-ignore will be fixed in a future moonlands update
                game.getMetaValue(action?.source, action.generatedBy)
              : action.target;
        ...
        case EFFECT_TYPE_ENERGY_DISCARDED_FROM_CREATURE: {
          ...
          const target =
            targetCard instanceof Array
              ? targetCard.map(convertCardMinimal)
              : convertCardMinimal(targetCard);

          return {
            ...action,
            target,
            source: action.source
              ? convertCardMinimal(action.source)
              : action.source,
            triggerSource: action.triggerSource
              ? convertCardMinimal(action.triggerSource)
              : action.triggerSource,
            amount,
          } as ClientEffectEnergyDiscardedFromCreature;
        }
        case EFFECT_TYPE_REMOVE_ENERGY_FROM_CREATURE: {
          const targetCard: CardInGame | CardInGame[] =
            typeof action.target == "string"
              ? game.getMetaValue(action.target, action.generatedBy)
              : action.target;
        ...
        case EFFECT_TYPE_ADD_ENERGY_TO_CREATURE: {
          ...
          if (
            !(targetCard instanceof Array) &&
            (!("_card" in targetCard) || !targetCard._card)
          ) {
        ...
        case EFFECT_TYPE_ADD_ENERGY_TO_MAGI: {
          const targetCard =
            typeof action.target == "string"
              ...
              : action.target;
          // @ts-ignore
        //   const sourceCard: CardInGame | undefined =
        //     typeof action.source == "string"
        //       ? // @ts-ignore
        //         game.getMetaValue(action.source, action.generatedBy)
        //       : action.target;
        ...
        case EFFECT_TYPE_DISTRIBUTE_ENERGY_ON_CREATURES: {
          const energyOnCreatures: Record<string, number> = game.getMetaValue(
            action.energyOnCreatures,
            action.generatedBy,
          );
          return {
            type: ACTION_EFFECT,
        ...
        case EFFECT_TYPE_CREATE_CONTINUOUS_EFFECT: {
          const staticAbilities = (action.staticAbilities || []).map(
            (ability) =>
              Object.fromEntries(
                Object.entries(ability).map(([k, v]) => [
                  k,
                  game.getMetaValue(v, action.generatedBy),
                ]),
              ),
          );
          return {
            ...
            expiration: {
              ...action.expiration,
              turns:
                typeof action.expiration.turns == "string"
                  ? game.getMetaValue(
                      action.expiration.turns,
                      action.generatedBy,
                    )
                  : action.expiration.turns,
            },
        ...
        case EFFECT_TYPE_DISCARD_ENERGY_FROM_CREATURE_OR_MAGI: {
          ...
          const source =
            sourceCard && "length" in sourceCard && sourceCard.length
              ? sourceCard[0]
              : sourceCard;
          return {
            type: action.type,
        ...
        case EFFECT_TYPE_ATTACH_CARD_TO_CARD: {
          ...
          const attachmentTarget =
            typeof action.attachmentTarget == "string"
              ? game.getMetaValue<CardInGame>(
...
export function convertClientCommands(
  action: ClientAction,
  game: State,
): AnyEffectType | null {
  switch (action.type) {
    case ACTION_RESOLVE_PROMPT: {
      switch (game.state.promptType) {
        ...
        case PROMPT_TYPE_POWER_ON_MAGI: {
          if (
            action.power &&
            game.state.promptParams?.magi &&
            game.state.promptParams?.magi.length
          ) {
            return {
              type: action.type,
              power: game.state.promptParams?.magi[0].card.data.powers?.find(
                (power) => power.name === action.power,
              ),
        ...
        case PROMPT_TYPE_SINGLE_CREATURE_OR_MAGI: {
          if (action.target) {
            let target = game
              .getZone(ZONE_TYPE_IN_PLAY, null)
              .byId(action.target);
            if (!target) {
              target = game
                .getZone(ZONE_TYPE_ACTIVE_MAGI, game.players[0])
                .byId(action.target);
            }
            if (!target) {
              target = game
                .getZone(ZONE_TYPE_ACTIVE_MAGI, game.players[1])
                .byId(action.target);
            }
            if (target) {
              return {
        ...
        case PROMPT_TYPE_CHOOSE_UP_TO_N_CARDS_FROM_ZONE: {
          if (action.zone) {
            const zone =
              action.zone === ZONE_TYPE_IN_PLAY
                ? game.getZone(ZONE_TYPE_IN_PLAY)
                : game.getZone(action.zone, action.zoneOwner);
            const zoneContent = zone.cards;
            const actionCards = action.cards;
            if (actionCards) {
              const cards = zoneContent.filter((card) =>
                actionCards.includes(card.id),
              );
              return {
                type: action.type,
                cards,
        ...
        case PROMPT_TYPE_CHOOSE_N_CARDS_FROM_ZONE: {
          const zone =
            game.state.promptParams.zone === ZONE_TYPE_IN_PLAY
              ? game.getZone(ZONE_TYPE_IN_PLAY)
              : game.getZone(
                  game.state.promptParams.zone || ZONE_TYPE_HAND,
                  game.state.promptParams.zoneOwner || 0,
                );
          const zoneContent = zone.cards;
          const actionCards = action.cards;
          if (actionCards) {
    ...
    case ACTION_POWER: {
      if (action.source && action.power && typeof action.source === "string") {
        ...
        const power = powerSource.card.data.powers.find(
          (power) => power.name === action.power,
        );
    ...
    case ACTION_ATTACK: {
      const attackSource = game
        .getZone(ZONE_TYPE_IN_PLAY, null)
        .byId(action.source);
      if (!attackSource) {
        return null;
      }
      let attackTarget = game
        .getZone(ZONE_TYPE_IN_PLAY, null)
        .byId(action.target);
      let additionalAttackers: CardInGame[] = [];

      if (action.additionalAttackers) {
        additionalAttackers = action.additionalAttackers.map((id) =>
          game.getZone(ZONE_TYPE_IN_PLAY, null).byId(id),
        ) as CardInGame[];
      }

      if (!attackTarget) {
        const controller = game.modifyByStaticAbilities(
          attackSource,
          PROPERTY_CONTROLLER,
        );
        const opponentId = game.getOpponent(controller);
        attackTarget = game
          .getZone(ZONE_TYPE_ACTIVE_MAGI, opponentId)
          .byId(action.target);
      }
    ...
    case ACTION_PLAY: {
      ...
      const cardInHand = game
        .getZone(ZONE_TYPE_HAND, player)
        .byId(action.payload.card.id);
