...
import {
  affectAddEnergy,
  affectRemoveEnergy,
  cleanupContinuousEffects,
  findInPlay,
  getZoneName,
  tickDownContinuousEffects,
} from "./utils.js";
...
const zonesToConsiderForStaticAbilities = new Set<string>([
  "inPlay",
  "opponentInPlay",
  "playerActiveMagi",
  "opponentActiveMagi",
...
export function applyEffect(state: State, action: ClientEffectAction): State {
  switch (action.effectType) {
    ...
    case EFFECT_TYPE_CARD_MOVED_BETWEEN_ZONES: {
      const sourceZone: keyof State["zones"] = getZoneName(
        action.sourceZone,
        action.sourceCard,
      );
      const destinationZone: keyof State["zones"] = getZoneName(
      ...
      if (
        (sourceZone === "playerHand" || sourceZone === "opponentHand") &&
        destinationZone === "inPlay"
      ) {
      ...
      return {
        ...
        zones: {
          ...state.zones,
          [sourceZone]: (state.zones[sourceZone] as ConvertedCard[]).filter(
            (card: ConvertedCard) => card.id !== action.sourceCard.id,
          ),
          [destinationZone]: [
    ...
    case EFFECT_TYPE_END_OF_TURN: {
      return {
        ...state,
        turnTimer: false,
        continuousEffects: cleanupContinuousEffects(
          state.continuousEffects,
          action.player === 2,
        ),
      };
    ...
    case EFFECT_TYPE_REMOVE_ENERGY_FROM_MAGI: {
      ...
      const opponentActiveMagi = [
        ...
        card.id == action.target.id
          ? {
              ...card,
              data: { ...card.data, energy: card.data.energy - action.amount },
            }
          : card,
      );
      ...
      const inPlay = [...(state.zones.inPlay || [])].map((card) =>
        idsToFind.includes(card.id)
          ? {
              ...
              data: {
                ...card.data,
                energy: affectRemoveEnergy(state, card, action),
              },
            }
          : card,
      );
    ...
    case EFFECT_TYPE_ENERGY_DISCARDED_FROM_CREATURE: {
      ...
      return {
        ...
        energyLosses: [
          ...state.energyLosses,
          ...idsToFind.map((id, index) => ({
            value: -action.amount,
            card: id,
            ttl: 2,
            id: state.energyLossId + index,
          })),
        ],
      ...
      if (action.zone === ZONE_TYPE_DECK) {
        switch (action.zoneOwner) {
          case state.activePlayer: {
            const cardsToRearrange = state.zones.playerDeck.slice(
              0,
              action.cards.length,
            );
          ...
          default: {
            const cardsToRearrange = state.zones.opponentDeck.slice(
              0,
              action.cards.length,
            );
            const cardsRecord: Record<string, HiddenConvertedCard> = {};
    ...
    case EFFECT_TYPE_ENERGY_DISCARDED_FROM_MAGI: {
      ...
      const playerActiveMagi = [...state.zones.playerActiveMagi].map((card) =>
        card.id == action.target.id
          ? {
              ...card,
              data: { ...card.data, energy: card.data.energy - action.amount },
      ...
      return {
        ...
        energyLosses: [
          ...
          {
            value: -action.amount,
            card: action.target.id,
            ttl: 2,
            id: state.energyLossId,
          },
        ],
    ...
    case EFFECT_TYPE_ADD_ENERGY_TO_CREATURE: {
      ...
      const inPlay = [...(state.zones.inPlay || [])].map((card) =>
        idsToFind.includes(card.id)
          ? {
              ...
              data: {
                ...card.data,
                energy: affectAddEnergy(state, card, action),
              },
            }
          : card,
      );
      ...
      return {
        ...
        energyLosses: shouldShow
          ...
          : state.energyLosses,
        energyLossId: shouldShow
          ? state.energyLossId + idsToFind.length
          : state.energyLossId,
      };
    }
    case EFFECT_TYPE_MOVE_ENERGY: {
            ...
            ids.includes(cardInPlay.id)
              ? {
                  ...
                  data: {
                    ...cardInPlay.data,
                    energy:
                      action.energyOnCreatures[cardInPlay.id] ||
                      cardInPlay.data.energy,
                  },
                }
              : cardInPlay,
          ),
