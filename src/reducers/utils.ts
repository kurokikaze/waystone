...
export const tickDownContinuousEffects = (
  effects: ContinuousEffectType[],
  opponent: boolean,
...
export const cleanupContinuousEffects = (
  effects: ContinuousEffectType[],
  opponent: boolean,
): ContinuousEffectType[] => {
  let resultingEffects: ContinuousEffectType[] = [];
...
export const affectRemoveEnergy = (
  state: State,
  card: ConvertedCard,
  action: ClientEffectRemoveEnergyFromCreature,
): number => {
  const gameCard = byName(card.card);
  const source = action.source ? findInPlay(state, action.source.id) : null;
  if (
    gameCard &&
    gameCard.data.energyStasis &&
    source &&
    source.data.controller === card.data.controller
  ) {
    // For the Colossus
  ...
  return energyLossThreshold > 0
    ? Math.min(energyLoss, energyLossThreshold)
    : energyLoss;
};

export const affectAddEnergy = (
  state: State,
  card: ConvertedCard,
  action: ClientEffectAddEnergyToCreature,
): number => {
  const gameCard = byName(card.card);
  const source = action.source ? findInPlay(state, action.source.id) : null;
  if (
