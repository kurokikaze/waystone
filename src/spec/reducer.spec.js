...
describe("Separating static abilities into state property", () => {
  it("Static abilities are added correctly", () => {
    ...
    expect(resultingState.staticAbilities.length).toEqual(
      1,
      "One new static ability added",
    );
    expect(
      resultingState.staticAbilities[0].card.data.staticAbilities.length,
    ).toEqual(1, "Card data is included in static ability store");
  });
  ...
  it("Static abilities are removed correctly", () => {
    ...
    expect(thirdState.staticAbilities.length).toEqual(
      0,
      "Static ability is removed if card has moved out",
    );
  });
  ...
  it("Static abilities are not added if they are not needed", () => {
    ...
    expect(resultingState.staticAbilities.length).toEqual(
      0,
      "No static abilities added if the card doesnt have them",
    );
  });
  ...
  it("Rearrange energy reducer", () => {
    ...
    expect(resultingState.promptType).toEqual(
      PROMPT_TYPE_REARRANGE_ENERGY_ON_CREATURES,
    );
