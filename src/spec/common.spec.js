...
describe("Selectors", () => {
  ...
  it("Static abilities affecting cards", () => {
    ...
    expect(result.inPlay[0].data.affectedBy[0].name).toEqual(
      "Test control ability",
    );
    expect(result.inPlay[0].data.affectedBy[0].text).toEqual(
      "Control change ability",
    );
  });
  ...
  it("Correct application of layers", () => {
    ...
    expect(result.inPlay[0].data.affectedBy[1].name).toEqual(
      "Test control ability",
    );
    expect(result.inPlay[0].data.affectedBy[1].text).toEqual(
      "Control change ability",
    );

    expect(result.inPlay[0].data.affectedBy[0].name).toEqual("Empower");
    expect(result.inPlay[0].data.affectedBy[0].text).toEqual(
      "Powers on Creatures you control cost one less to a minimum of one",
    );
  });
  ...
  it("Game statis abilities", () => {
    ...
    expect(result.inPlay[0].data.affectedBy[0].name).toEqual(
      "Burrowed - Energy loss",
    );
    expect(result.inPlay[0].data.affectedBy[0].text).toEqual(
      "Your burrowed creatures cannot lose more than 2 energy each turn",
    );

    expect(result.inPlay[0].data.affectedBy[1].name).toEqual(
      "Burrowed - Ability to attack",
    );
    expect(result.inPlay[0].data.affectedBy[1].text).toEqual(
      "Your burrowed creatures cannot attack",
    );
  });
});
