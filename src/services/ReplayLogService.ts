...
export class ReplayLogService {
  ...
  public async getReplaysList() {
    if (this.isTauri()) {
      const entries = await readDir(ReplayLogService.REPLAYS_DIR, {
  ...
  public async readReplay(replay: string): Promise<ClientMessage[]> {
    if (this.isTauri()) {
      const contentRaw = await readTextFile(
        `${ReplayLogService.REPLAYS_DIR}\\${replay}`,
    ...
    return new Promise((resolve) =>
      resolve(
        (replay === "testReplay" ? testReplay : testReplay2) as ClientMessage[],
      ),
    );
  }
  ...
  private async createReplayFileIfNotExists(
    ...
    const deckFileExists = await exists(
      `${ReplayLogService.REPLAYS_DIR}\\${replayName}.log`,
      { baseDir: BaseDirectory.AppConfig },
    );
    if (!deckFileExists) {
