...
export function getMaxPaymentSourceEnergy(state: State) {
  const activeMagi = zoneContent("playerActiveMagi", state);
  const paymentCardEnergies = state.zones.inPlay
    .filter((card) => {
  ...
  const fullArray: number[] =
    activeMagi.length && activeMagi[0].data?.energy
      ? [activeMagi[0].data?.energy, ...paymentCardEnergies]
      : paymentCardEnergies;
...
export const isPRSAvailable = (state: State) =>
  state.activePlayer == 1 &&
  typeof state.step == "number" &&
  [STEP_PRS_FIRST, STEP_PRS_SECOND].includes(state.step);

export const getActivePlayerMagi = (state: State) =>
  state.zones.playerActiveMagi[0];
export const getPromptCards = (state: State) => {
  return state.promptParams?.cards;
};
export const getStartingCards = (state: State) =>
  state.promptParams?.startingCards;
export const getAvailableCards = (state: State) =>
  state.promptParams?.availableCards;
export const getPromptGeneratedBy = (state: State) => state.promptGeneratedBy;
export const getPromptNumberOfCards = (state: State) =>
  state.promptParams?.numberOfCards;
export const getPromptMagi = (state: State) => state.promptParams?.magi;
export const getPromptMin = (state: State) => {
  return typeof state.promptParams?.min == "number"
    ? state.promptParams?.min
    : 1;
};
...
export const getPromptZoneOwner = (state: State) =>
  state.promptParams?.zoneOwner;
export const getPromptMessage = (state: State) => state.promptMessage;
export const getPromptType = (state: State) =>
  state.prompt ? state.promptType : null;
...
export const getAlternatives = (state: State) =>
  state.promptParams?.alternatives || [];
export const getMyRelicNames = (state: State) =>
  state.zones.inPlay
    .filter(
      (cardData) => cardData.data.controller === 1 && relicsHash[cardData.card],
    )
    .map((cardData) => cardData.card);
export const getDefeatedCreatureId = (state: State) =>
  state.animation && state.animation.type === ANIMATION_CREATURE_DISCARDED
    ? state.animation.target
    : null;
export const getCardsCountInOurDiscard = (state: State) =>
  state.zones.playerDiscard.length;
export const getCardsCountInOpponentDiscard = (state: State) =>
  state.zones.opponentDiscard.length;
export const getCardsCountInOurDeck = (state: State) =>
  state.zones.playerDeck.length;
export const getCardsCountInOpponentDeck = (state: State) =>
  state.zones.opponentDeck.length;
export const getAnimation = (state: State) => state.animation;
export const getIsOnMagiPrompt = (state: State) =>
  state.prompt &&
