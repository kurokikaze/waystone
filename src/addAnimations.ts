  ...
  switch (action.type) {
    case ACTION_RESOLVE_PROMPT:
      ...
      return action.player !== 1 || !onlyEnemy
        ? [
            startPromptResolutionAnimation(
              action.target
                ? getPowerSource(action.target)(state)?.card || ""
                : (action.number || 0).toString(),
            ),
            endPromptResolutionAnimation(),
            action,
          ]
    ...
    case ACTION_ATTACK: {
      if (action.player !== 1 || !onlyEnemy) {
        if (action.player == 1) {
          console.log(
            `Adding startAttackAnimation from ${action.source} to ${action.target}`,
          );
        }
      }
      return action.player !== 1 || !onlyEnemy
        ? [
            startAttackAnimation(
              action.source,
              action.target,
              action.additionalAttackers && action.additionalAttackers.length
                ? action.additionalAttackers[0]
                : null,
              action.player,
            ),
    ...
    case ACTION_EFFECT: {
      switch (action.effectType) {
        ...
        case EFFECT_TYPE_START_STEP:
          return [endStepAnimation(), action];
        case EFFECT_TYPE_MAGI_IS_DEFEATED: {
          return [startMagiDefeatAnimation(action.target.id), action];
        }
        case EFFECT_TYPE_DISCARD_CREATURE_FROM_PLAY: {
          return [startCreatureDiscardAnimation(action.target.id), action];
        }
      }
      return [action];
    }
...
export default function addAnimationsNew(
  action$: Observable<ClientCommand>,
  break$: Observable<{}>,
  store: Store<any, Action<any>>,
  onlyEnemy = true,
) {
