      ...
      if (currentStep !== 5 && this.gameState.playerPriority(this.playerId)) {
        ...
        switch (this.gameState.getPromptType()) {
          case PROMPT_TYPE_CHOOSE_N_CARDS_FROM_ZONE: {
            ...
            const resultCards = cards?.slice(
              0,
              this.gameState.state.promptParams.numberOfCards,
            );
            postMessage({
          ...
          default:
            console.log(
              `We are prompted outside of our turn, the prompt type is ${this.gameState.getPromptType()}`,
            );
        }
      } else if (
        currentStep == 5 &&
        ...
        switch (this.gameState.getPromptType()) {
          ...
          default: {
            console.log(
              `We are prompted on the DRAW step, the prompt type is ${this.gameState.getPromptType()}`,
            );
          }
        }
      }
