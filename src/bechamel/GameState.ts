...
const zonesToConsiderForStaticAbilities = new Set([
  "inPlay",
  "opponentInPlay",
  "playerActiveMagi",
  "opponentActiveMagi",
...
type ProcessedCardFilter = (
  card: ProcessedClientCard,
  id: number,
  arr: ProcessedClientCard[],
) => boolean;
...
export const findInPlay = (state: StateRepresentation, id: string) => {
  ...
  const cardPlayerMagi = state.zones.playerActiveMagi.find(
    (card) => card.id === id,
  );
  if (cardPlayerMagi) return cardPlayerMagi;

  const cardOpponentMagi = state.zones.opponentActiveMagi.find(
    (card) => card.id === id,
  );
...
export class GameState {
  ...
  public waitingForCardSelection(): boolean {
    return (
      this.state.prompt && this.state.promptType === PROMPT_TYPE_CHOOSE_CARDS
    );
  }

  public waitingForPaymentSourceSelection(): boolean {
    return (
      this.state.prompt && this.state.promptType === PROMPT_TYPE_PAYMENT_SOURCE
    );
  }
  ...
  public waitingForTarget(byId: string, playerId: number): boolean {
    return (
      this.isInPromptState(playerId) && this.state.promptGeneratedBy === byId
    );
  }
  ...
  public getPlayableCards(): ClientCard[] {
    ...
    return this.state.zones.playerHand.filter((card) => {
      ...
      if (
        cardData.type === TYPE_RELIC &&
        cardData.region !== magiCard.region &&
        cardData.region !== REGION_UNIVERSAL
      )
        return false;
  ...
  public getMyRelicsInPlay(): ProcessedClientCard[] {
    ...
    return realState.inPlay
      ...
      .filter<ProcessedClientCard>(
        (card): card is ProcessedClientCard =>
          card._card?.type === TYPE_RELIC &&
          card.data.controller === this.playerId,
      );
  }
      ...
      switch (this.state.promptParams.restriction) {
        case RESTRICTION_CREATURE_NAME: {
          return (card: ProcessedClientCard) =>
            card.card.name === this.state.promptParams.restrictionValue;
        }
        default: {
          return () => false;
        }
  ...
  public getEnemyCreaturesInPlay(): ProcessedClientCard[] {
    ...
    return realState.inPlay
      ...
      .filter<ProcessedClientCard>((card): card is ProcessedClientCard =>
        Boolean(
          card._card?.type === TYPE_CREATURE &&
            card.data.controller !== this.playerId,
        ),
  ...
  public getEnemyRelicsInPlay(): ProcessedClientCard[] {
    ...
    return realState.inPlay
      ...
      .filter<ProcessedClientCard>(
        (card): card is ProcessedClientCard =>
          card._card?.type === TYPE_RELIC && card.owner !== this.playerId,
      );
  }
  ...
  private applyEffect(
    ...
    switch (action.effectType) {
      case EFFECT_TYPE_CARD_MOVED_BETWEEN_ZONES: {
        const sourceZone = this.getZoneName(
          action.sourceZone,
          action.sourceCard,
        );
        const destinationZone = this.getZoneName(
        ...
        if (zonesToConsiderForStaticAbilities.has(sourceZone)) {
          ...
          staticAbilities = staticAbilities.filter(
            (card) => card.id !== action.sourceCard.id,
          );
        } else if (
          zonesToConsiderForStaticAbilities.has(destinationZone) &&
          action.destinationCard.card &&
          byName(action.destinationCard.card)?.data.staticAbilities
        ) {
        ...
        const sourceZoneContent: (ClientCard | HiddenCard)[] =
          state.zones[sourceZone];
        ...
        return {
          ...
          zones: {
            ...state.zones,
            [sourceZone]: sourceZoneContent.filter(
              (card: { id: string }): boolean =>
                card.id !== action.sourceCard.id,
            ),
            [destinationZone]: [
              ...state.zones[destinationZone],
              action.destinationCard,
            ],
          },
        };
        ...
        if (action.player === this.playerId) {
          return {
            ...
            zones: {
              ...
              playerActiveMagi: state.zones.playerActiveMagi.map((card) => ({
                ...card,
                data: { ...card.data, wasAttacked: false, actionsUsed: [] },
              })),
            },
            activePlayer: action.player,
            continuousEffects: tickDownContinuousEffects(
      ...
      case EFFECT_TYPE_REMOVE_ENERGY_FROM_MAGI: {
        const playerActiveMagi = [...(state.zones.playerActiveMagi || [])].map(
          (card) =>
      ...
      case EFFECT_TYPE_REMOVE_ENERGY_FROM_CREATURE: {
        const idsToFind =
          action.target instanceof Array
            ? action.target.map(({ id }) => id)
            : [action.target.id];

        const inPlay = [...(state.zones.inPlay || [])].map((card) =>
          idsToFind.includes(card.id)
            ? {
                ...card,
                data: {
                  ...card.data,
        ...
        const inPlay = [...state.zones.inPlay].map((card) =>
          idsToFind.includes(card.id)
            ? {
                ...
                data: {
                  ...card.data,
                  energy: Math.max(card.data.energy - action.amount, 0),
                },
              }
            : card,
        );
      ...
      case EFFECT_TYPE_ENERGY_DISCARDED_FROM_MAGI: {
        ...
        const opponentActiveMagi = [...state.zones.opponentActiveMagi].map(
          (card) =>
            card.id == action.target.id
              ? {
                  ...card,
      ...
      case EFFECT_TYPE_REARRANGE_ENERGY_ON_CREATURES: {
        ...
        return {
          ...
          zones: {
            ...state.zones,
            inPlay: state.zones.inPlay.map((cardInPlay) =>
              ids.includes(cardInPlay.id)
                ? {
                    ...cardInPlay,
                    data: {
                      ...cardInPlay.data,
                      energy: action.energyOnCreatures[cardInPlay.id],
                    },
                  }
                : cardInPlay,
            ),
      ...
      case EFFECT_TYPE_DISCARD_RESHUFFLED: {
        const newState: StateRepresentation =
          action.player === this.playerId
            ? {
                ...
                zones: {
                  ...state.zones,
                  playerDiscard: [],
                  playerDeck: action.cards.map((cardId) => ({
                    id: cardId,
                    owner: action.player,
                    card: null,
                    data: {},
            ...
            : {
                ...
                zones: {
                  ...state.zones,
                  opponentDiscard: [],
                  opponentDeck: action.cards.map((cardId) => ({
                    id: cardId,
                    owner: action.player,
                    card: null,
                    data: {},
