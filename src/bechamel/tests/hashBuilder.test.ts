...
describe("hashBuilder", () => {
  it("builds hashes", () => {
    ...
    const welliskPup = new CardInGame(
      byName("Wellisk Pup") as Card,
      ACTIVE_PLAYER,
    ).addEnergy(2);
    const seaBarl = new CardInGame(
      byName("Sea Barl") as Card,
      ACTIVE_PLAYER,
    ).addEnergy(4);

    const orlon = new CardInGame(
      byName("Orlon") as Card,
      ACTIVE_PLAYER,
    ).addEnergy(7);
    const bwill = new CardInGame(
      byName("Bwill") as Card,
      ACTIVE_PLAYER,
    ).addEnergy(4);

    const nimbulo = new CardInGame(
      byName("Nimbulo") as Card,
      NON_ACTIVE_PLAYER,
    ).addEnergy(5);
    const zones = createZones(
      ACTIVE_PLAYER,
      NON_ACTIVE_PLAYER,
      [welliskPup, seaBarl, bwill],
      [orlon],
    );
    ...
    expect(builder.makeHash(gameState)).toEqual(
      "*4{}@7[]|#1(0/1):2|#2(0/1):4|#3(0/1):4|@5",
    );
  });
  ...
  it("follows summons", () => {
    ...
    const handOne = gameState
      .getZone(ZONE_TYPE_HAND, ACTIVE_PLAYER)
      .cards.map(({ id }) => id);
    const handTwo = secondSim
      .getZone(ZONE_TYPE_HAND, ACTIVE_PLAYER)
      .cards.map(({ id }) => id);
    ...
    const handOneAfter = gameState
      .getZone(ZONE_TYPE_HAND, ACTIVE_PLAYER)
      .cards.map(({ id }) => id);
    const handTwoAfter = secondSim
      .getZone(ZONE_TYPE_HAND, ACTIVE_PLAYER)
      .cards.map(({ id }) => id);
  ...
  it("diamond-shaped actions", () => {
    ...
    const cardOneTwo = gameState.getZone(ZONE_TYPE_HAND, ACTIVE_PLAYER)
      .cards[0];
    // console.dir(card);
    ...
    const cardTwoTwo = secondSim.getZone(ZONE_TYPE_HAND, ACTIVE_PLAYER)
      .cards[0];
    // console.dir(card);
