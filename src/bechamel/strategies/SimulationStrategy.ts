...
import {
  ...
  FromClientPassAction,
  FromClientPlayAction,
  FromClientPowerAction,
} from "../../clientProtocol";
import {
...
export class SimulationStrategy implements Strategy {
    ...
    switch (simAction.type) {
      case ACTION_PLAY: {
        if ("payload" in simAction) {
          return this.play(
            simAction.payload.card.id,
            `Play the card ${simAction.payload.card.card.name}`,
          );
        }
        break;
      }
      ...
      case ACTION_ATTACK: {
        const add = simAction.additionalAttackers
          ? simAction.additionalAttackers[0]?.id
          : "";
        if (
          simAction.source instanceof CardInGame &&
          simAction.target instanceof CardInGame
        ) {
      ...
      case ACTION_RESOLVE_PROMPT: {
        if (
          ...
          return this.resolveTargetPrompt(
            simAction.target.id,
            simAction.target.card.name,
          );
        }
        ...
        if ("cards" in simAction && simAction.cards instanceof Array) {
          return this.resolveCardsPrompt(
            simAction.cards,
            "",
            // @ts-ignore No zone field on some prompt resolvers
            simAction?.zone,
            // @ts-ignore No zoneOwner field on some prompt resolvers
            simAction?.zoneOwner,
          );
        }
  ...
  private simulateAttacksQueue(
    simulationQueue: SimulationEntity[],
    initialScore: number,
    opponentId: number,
  ): AnyEffectType {
    ...
    while (simulationQueue.length && failsafe > 0) {
      ...
      if (workEntity) {
        ...
        if (score > bestAction.score) {
          ...
          bestAction.action =
            workEntity?.actionLog.map(({ action }) => action) || [];
        }
  ...
  private simulateActionsQueue(
    ...
    while (simulationQueue.length && counter <= SimulationStrategy.failsafe) {
      ...
      if (workEntity && workEntity.action) {
        ...
        try {
          this.graph =
            this.graph +
            `  "${workEntity.previousHash}" -> "${hash}" [label="${this.actionToLabel(workEntity.action)}"]\n`;
        } catch (_e) {
    ...
    this.leaves.forEach((value: Leaf) => {
      if (
        (!value.isPrompt && value.score > bestAction.score) ||
        (value.score == bestAction.score &&
          value.actionLog.length < bestAction.actions.length)
      ) {
  ...
  private shouldClearHeldActions(): boolean {
    ...
    if (this.gameState.getStep() === STEP_NAME.CREATURES) {
      ...
      if (
        ...
        console.log(
          `Failed summon, passing. Dropping ${this.actionsOnHold.length} actions on hold.`,
        );
        return true;
      }
    } else if (
      this.gameState.getStep() === STEP_NAME.PRS1 ||
      this.gameState.getStep() === STEP_NAME.PRS2
    ) {
      ...
      if (action.type === ACTION_POWER && !ids.has(action.source)) {
        console.log(
          `Failed power activation, no card with id ${action.source} (power to be activated is ${action.power}). Dropping ${this.actionsOnHold.length} actions on hold.`,
        );
    ...
    if (
      this.gameState.isInMyPromptState() &&
  ...
  private fixTargetPromptResolution(
    action: C2SAction & { targetName: string },
  ): C2SAction {
    const promptAvailableCards = this.gameState?.state.promptParams.cards?.map(
      ({ id }) => id,
    );
  ...
  private fixCardPromptResolution(action: C2SAction): C2SAction {
    ...
    if (
      ...
      action.cards &&
      action.cards.some(
        (card) => !promptAvailableCards?.includes(card as unknown as string),
      )
    ) {
      ...
      for (const cardName of action.cardNames) {
        if (
          !(cardName in availableCardPairs) ||
          availableCardPairs[cardName].length == 0
        ) {
          console.dir(action);
  ...
  public requestAction(): C2SAction {
    ...
    if (this.actionsOnHold.length) {
      ...
      if (
        ...
        this.gameState.getStep() === STEP_NAME.CREATURES
      ) {
        this.actionsOnHold = [];
      }
      // if (checkHash !== hash) {
      ...
      if (
        ...
        !this.gameState.state.promptParams.cards?.some(
          ({ id }) => id == action.target,
        )
      ) {
        ...
        return this.fixTargetPromptResolution(
          action as ClientResolvePromptAction & { targetName: string },
        );
      }
      return action;
    }
    ...
    if (this.gameState && this.playerId) {
      ...
      if (
        this.gameState.isInPromptState(this.playerId) &&
        this.gameState.getPromptType() === PROMPT_TYPE_MAY_ABILITY
      ) {
        const myMagi = this.gameState.getMyMagi();
        if (
          myMagi.card === "Stradus" &&
          this.gameState.state.promptGeneratedBy === myMagi.id
        ) {
          return {
            type: ACTION_RESOLVE_PROMPT,
        ...
        switch (step) {
          ...
          case STEP_NAME.PRS2: {
            if (this.gameState.isInMyPromptState()) {
              ...
              console.log(
                `Prompt state without previous action: ${this.gameState.getPromptType()}`,
              );
            }
            ...
            const initialScore = getStateScore(
              outerSim,
              this.playerId,
              TEMPORARY_OPPONENT_ID,
            );
            ...
            const bestActions = this.simulateActionsQueue(
              simulationQueue,
              initialScore,
              TEMPORARY_OPPONENT_ID,
            );
            ...
            if (finalHash !== hash) {
              console.log(
                `Change leak! hashes mismatch: ${hash} => ${finalHash}`,
              );
            }
            ...
            this.actionsOnHold = bestActions
              .slice(1)
              .filter(
                ({ action }) =>
                  (action.type === ACTION_PLAY &&
                    "payload" in action &&
                    action.payload.player === this.playerId) ||
          ...
          case STEP_NAME.CREATURES: {
            ...
            const playable = this.gameState
              ...
              .filter((card) => {
                const regionTax =
                  myMagi._card.region === card._card.region ? 0 : 1;
                return (
          ...
          case STEP_NAME.ATTACK: {
            ...
            if (opponentMagi) {
              ...
              const hash = this.hashBuilder.makeHash(outerSim);
              const simulationQueue = ActionExtractor.extractActions(
                outerSim,
                this.playerId,
                TEMPORARY_OPPONENT_ID,
                [],
              ...
              const bestAction = this.simulateAttacksQueue(
                simulationQueue,
                initialScore,
                TEMPORARY_OPPONENT_ID,
              );
              ...
              if (
                bestAction.type === ACTION_ATTACK ||
                bestAction.type === ACTION_RESOLVE_PROMPT
              ) {
            ...
            if (this.gameState.isInMyPromptState()) {
              ...
              throw new Error(
                `Unexpected prompt type in ATTACK step: ${this.gameState.getPromptType()}`,
              );
            }
            return this.pass();
          }
  ...
  resolveChooseCardsPrompt() {
    ...
    if (
      ...
      console.dir(
        this.resolveCardsPrompt(
          availableCards
            .slice(0, this.gameState.state.promptParams.numberOfCards || 0)
