...
export class ActionExtractor {
  public static extractActions(
    ...
    if (sim.state.prompt) {
      return ActionExtractor.extractPromptAction(
        sim,
        playerId,
        opponentId,
        actionLog,
    ...
    switch (step) {
      ...
      case STEP_NAME.PRS1:
      case STEP_NAME.PRS2: {
        const magiCard: CardInGame | null = sim.getZone(
          ZONE_TYPE_ACTIVE_MAGI,
          playerId,
        ).card;
        ...
        const mySimCreatures = (
          sim.getZone(ZONE_TYPE_IN_PLAY).cards as CardInGame[]
        ).filter(
          (card) =>
            sim.modifyByStaticAbilities(card, PROPERTY_CONTROLLER) === playerId,
        );
        const creaturesWithPowers = mySimCreatures.filter(
          (creature) =>
            creature.card.data.powers && creature.data.actionsUsed.length === 0,
        );

        const simulationQueue: SimulationEntity[] = [];

        simulationQueue.push(
          ActionExtractor.getPassAction(sim, playerId, actionLog, previousHash),
        );

        creaturesWithPowers.forEach((card) => {
          const power = card.card?.data?.powers
            ? card.card.data.powers[0]
            : null;
          // Don't forget: Relic powers are paid from the Magi
          const energyReserve =
            card.card.type === TYPE_CREATURE
              ? card.data.energy
              : magiCard.data.energy;
          if (
            power &&
            typeof power.cost == "number" &&
            power.cost <= energyReserve
          ) {
        ...
        if (magiCard) {
          if (magiCard.card.data.powers && magiCard.card.data.powers.length) {
            magiCard.card.data.powers.forEach((power: Record<string, any>) => {
              if (
                !magiCard.data.actionsUsed.includes(power.name) &&
                power.cost <= magiCard.data.energy
              ) {
                const innerSim = sim.clone();
                const source = innerSim.getZone(
                  ZONE_TYPE_ACTIVE_MAGI,
                  playerId,
                ).card;
                if (source) {
          ...
          const playableSpells = sim
            .getZone(ZONE_TYPE_HAND, playerId)
            .cards.filter(
              (card) =>
          ...
          playableSpells.forEach((spell) => {
            ...
            if (
              card &&
              typeof spell.card.cost == "number" &&
              spell.card.cost <= magiCard.data.energy
            ) {
              const action: AnyEffectType = {
                type: ACTION_PLAY,
                payload: {
      ...
      case STEP_NAME.ATTACK: {
        const attackPatterns = ActionExtractor.getAllAttackPatterns(
          sim,
          playerId,
          opponentId,
        );

        const workEntities: SimulationEntity[] = [
          ActionExtractor.getPassAction(sim, playerId, actionLog, previousHash),
        ];
        attackPatterns.forEach((pattern) => {
          ...
          const target =
            innerSim.getZone(ZONE_TYPE_IN_PLAY).byId(pattern.to) ||
            innerSim
              .getZone(ZONE_TYPE_ACTIVE_MAGI, opponentId)
              .byId(pattern.to);

          const additionalAttackers = pattern.add
            ? [innerSim.getZone(ZONE_TYPE_IN_PLAY).byId(pattern.add)]
            : [];
          if (source && target) {
      ...
      case STEP_NAME.CREATURES: {
        ...
        if (magiCard) {
          const playableCreatures = sim
            .getZone(ZONE_TYPE_HAND, playerId)
            .cards.filter((card) => card.card.type === TYPE_CREATURE);
          playableCreatures.forEach((creature) => {
            const regionTax: number =
              creature.card.region === magiCard.card.region ||
              creature.card.region === REGION_UNIVERSAL
                ? 0
                : 1;
            if (
              typeof creature.card.cost == "number" &&
              creature.card.cost + regionTax <= magiCard.data.energy
            ) {
              const innerSim = sim.clone();
              innerSim.setOnAction((action) => {
                if (
                  action.type === ACTION_EFFECT &&
                  ...
                  hashBuilder.registerChildHash(
                    action.sourceCard.id,
                    action.destinationCard.id,
                  );
                }
              });
              const card = innerSim
  ...
  public static extractPromptAction(
    sim: State,
    playerId: number,
    opponentId: number,
    actionLog: ActionOnHold[],
    ...
    switch (sim.state.promptType) {
      ...
      case PROMPT_TYPE_SINGLE_CREATURE_FILTERED: {
        const allCreatures: CardInGame[] = sim
          .getZone(ZONE_TYPE_IN_PLAY)
          .cards.filter((card: CardInGame) => card.card.type === TYPE_CREATURE);
        let filteredCreatures = allCreatures;
        if (sim.state.promptParams.restrictions) {
          const filter = sim.makeCardFilter(
            sim.state.promptParams.restrictions,
          );
      ...
      case PROMPT_TYPE_RELIC: {
        const allRelics: CardInGame[] = sim
          .getZone(ZONE_TYPE_IN_PLAY)
          .cards.filter((card: CardInGame) => card.card.type === TYPE_RELIC);
        const simulationQueue: SimulationEntity[] = [];
      ...
      case PROMPT_TYPE_ANY_CREATURE_EXCEPT_SOURCE: {
        const allCreatures: CardInGame[] = sim
          ...
          .cards.filter(
            (card: CardInGame) =>
              card.card.type === TYPE_CREATURE &&
              card.id !== sim.state.promptGeneratedBy,
          );
        const simulationQueue: SimulationEntity[] = [];
        allCreatures.forEach((creature) => {
          const innerSim = sim.clone();
      ...
      case PROMPT_TYPE_OWN_SINGLE_CREATURE: {
        ...
        const myCreatures: CardInGame[] = sim
          .getZone(ZONE_TYPE_IN_PLAY)
          .cards.filter(
            (card: CardInGame) =>
              card.card.type === TYPE_CREATURE &&
              sim.modifyByStaticAbilities(card, PROPERTY_CONTROLLER) ===
                promptPlayer,
          );
        const simulationQueue: SimulationEntity[] = [];
      ...
      case PROMPT_TYPE_SINGLE_MAGI: {
        const myMagi: CardInGame | null = sim.getZone(
          ZONE_TYPE_ACTIVE_MAGI,
          playerId,
        ).card;
        const simulationQueue: SimulationEntity[] = [];
        if (myMagi) {
          const innerSim = sim.clone();
          const newMyMagi = innerSim.getZone(
            ZONE_TYPE_ACTIVE_MAGI,
            playerId,
          ).card;
          if (newMyMagi) {
        ...
        const opponentMagi: CardInGame | null = sim.getZone(
          ZONE_TYPE_ACTIVE_MAGI,
          opponentId,
        ).card;
        if (opponentMagi) {
          const innerSim = sim.clone();
          const newOpponentMagi = innerSim.getZone(
            ZONE_TYPE_ACTIVE_MAGI,
            opponentId,
          ).card;
          if (newOpponentMagi) {
      ...
      case PROMPT_TYPE_POWER_ON_MAGI: {
        ...
        if (
          myMagi &&
          myMagi.card.data.powers &&
          myMagi.card.data.powers.length
        ) {
          const innerSim = sim.clone();
          const newMyMagiPowers = innerSim.getZone(
            ZONE_TYPE_ACTIVE_MAGI,
            playerId,
          )?.card?.card?.data?.powers;
      ...
      case PROMPT_TYPE_REARRANGE_CARDS_OF_ZONE: {
        ...
        const action: AnyEffectType = {
          ...
          cardsOrder:
            innerSim.state.promptParams?.cards?.map(({ id }) => id) || [],
          generatedBy: innerSim.state.promptGeneratedBy,
          player: innerSim.state.promptPlayer as number,
        };
        simulationQueue.push({
      ...
      case PROMPT_TYPE_DISTRIBUTE_DAMAGE_ON_CREATURES: {
        ...
        const ids: [string, number][] = enemyCreatures.map((card) => [
          card.id,
          card.data.energy,
        ]);
      ...
      case PROMPT_TYPE_REARRANGE_ENERGY_ON_CREATURES: {
        ...
        const ids: [string, number][] = myCreatures.map((card) => [
          card.id,
          card.data.energy,
        ]);
      ...
      case PROMPT_TYPE_MAGI_WITHOUT_CREATURES: {
        ...
        const iHaveCreatures: boolean =
          sim.useSelector(SELECTOR_CREATURES_OF_PLAYER, playerId).length > 0;
        ...
        const opponentHasCreatures: boolean =
          sim.useSelector(SELECTOR_CREATURES_OF_PLAYER, opponentId).length > 0;
      ...
      case PROMPT_TYPE_CHOOSE_N_CARDS_FROM_ZONE: {
        ...
        if (sim.state.promptParams.zone) {
          const zoneCards = sim.getZone(
            sim.state.promptParams.zone,
            sim.state.promptParams.zoneOwner,
          ).cards;
          ...
          for (let i = 0; i < numberOfVariants; i++) {
            const innerSim = sim.clone();
            const cards = innerSim
              .getZone(
                sim.state.promptParams.zone,
                sim.state.promptParams.zoneOwner,
              )
              .cards.slice(i * promptCards, promptCards);
            const action: AnyEffectType = {
              type: ACTION_RESOLVE_PROMPT,
      ...
      case PROMPT_TYPE_PAYMENT_SOURCE: {
        ...
        if (sim.state.promptParams.cards) {
          for (const card of sim.state.promptParams.cards) {
            ...
            const foundCreatureCard = innerSim
              .getZone(ZONE_TYPE_IN_PLAY)
              .byId(card.id);
            const foundMagiCard = innerSim
              .getZone(ZONE_TYPE_ACTIVE_MAGI, sim.state.promptPlayer)
              .byId(card.id);
  ...
  public static getAllAttackPatterns(
    sim: State,
    attacker: number,
    opponent: number,
  ): AttackPattern[] {
    const creatures = sim
      .getZone(ZONE_TYPE_IN_PLAY)
      .cards.filter((card: CardInGame) => card.card.type === TYPE_CREATURE);
    const attackers = creatures.filter(
      (card: CardInGame) =>
        sim.modifyByStaticAbilities(card, PROPERTY_CONTROLLER) === attacker &&
        sim.modifyByStaticAbilities(card, PROPERTY_ABLE_TO_ATTACK) === true,
    );
    const defenders = creatures.filter(
      (card: CardInGame) =>
        sim.modifyByStaticAbilities(card, PROPERTY_CONTROLLER) !== attacker &&
        sim.modifyByStaticAbilities(card, PROPERTY_CAN_BE_ATTACKED) === true,
    );

    const allOpponentCreatures = creatures.filter(
      (card: CardInGame) =>
        sim.modifyByStaticAbilities(card, PROPERTY_CONTROLLER) !== attacker,
    );

    const enemyMagi = sim.getZone(ZONE_TYPE_ACTIVE_MAGI, opponent).cards[0];
    const magiCanBeAttacked = sim.modifyByStaticAbilities(
      enemyMagi,
      PROPERTY_CAN_BE_ATTACKED,
    );
    ...
    for (let attacker of attackers) {
      const numberOfAttacks = sim.modifyByStaticAbilities(
        attacker,
        PROPERTY_ATTACKS_PER_TURN,
      );
      if (attacker.data.attacked < numberOfAttacks) {
        if (
          (!allOpponentCreatures.length ||
            attacker.card.data.canAttackMagiDirectly) &&
          enemyMagi &&
          enemyMagi.data.energy > 0 &&
          magiCanBeAttacked
        ) {
