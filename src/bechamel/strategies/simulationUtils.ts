...
export const booleanGuard = Boolean as any as <T>(
...
export const createZones = (
  ...
  new Zone("Player 1 active magi", ZONE_TYPE_ACTIVE_MAGI, player1).add(
    activeMagi,
  ),
...
export function createState(
  ...
  const myCreaturesCards = [...myCreatures, ...myRelics]
    .map((card) => {
      const creatureCard = byName(card.card.name);
      if (!creatureCard) {
        return false;
      }
      const cardInGame = new CardInGame(creatureCard, playerId).addEnergy(
        card.data.energy,
      );
    ...
    .filter(booleanGuard);
  const enemyCreaturesCards = [...enemyCreatures, ...enemyRelics]
    .map((card) => {
      const creatureCard = byName(card.card.name);
      if (!creatureCard) {
        return false;
      }
      const cardInGame = new CardInGame(creatureCard, opponentId).addEnergy(
        card.data.energy,
      );
      cardInGame.data.attacked = card.data.attacked;
      cardInGame.data.wasAttacked = card.data.wasAttacked;
  ...
  if (myMagi && myMagiActualCard) {
    ...
    const myMagiCard: CardInGame = new CardInGame(
      myMagiActualCard,
      playerId,
    ).addEnergy(myMagi.data.energy);
  ...
  sim.getZone(ZONE_TYPE_MAGI_PILE, playerId).add(
    myMagiPile
      .map((magi) => {
  ...
  if (opponentMagi && enemyMagiRealCard) {
    const enemyMagiCard: CardInGame = new CardInGame(
      enemyMagiRealCard,
      opponentId,
    ).addEnergy(opponentMagi.data.energy);
  ...
  const playableEnrichedCards = gameState
    .getPlayableCards()
    .map(addCardData)
    .filter((card) => card._card.type !== TYPE_RELIC);

  sim.getZone(ZONE_TYPE_HAND, playerId).add(
    playableEnrichedCards
      .map((card): CardInGame | false => {
  ...
  sim.getZone(ZONE_TYPE_DECK, playerId).add(
    myDeckCardIds.map((id) => {
  ...
  const seed =
    gameState.getTurn() * 100 + gameState.getStep() * 10 + gameState.playerId;
  sim.initiatePRNG(seed);
  return sim;
}

export const CARD_SCORE = 0.1;
export const getStateScore = (
  state: State,
  attacker: number,
  opponent: number,
): number => {
