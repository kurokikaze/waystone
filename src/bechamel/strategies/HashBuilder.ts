...
export class HashBuilder {
  ...
  public makeHash(sim: State): string {
    ...
    for (const card of battlefieldCards) {
      const attacks = sim.modifyByStaticAbilities(
        card,
        PROPERTY_ATTACKS_PER_TURN,
      );
      ...
      const attackPart =
        ...
        card.data.controller == sim.getActivePlayer()
          ? `(${attacked}/${attacks})`
          : "";
      const energyPart =
        card.card.type === TYPE_CREATURE ? card.data.energy : "*";
      let powersPart = "";
      ...
      cardHashes.push(
        `#${this.convertHash(card.id)}${attackPart}${powersPart}:${energyPart}`,
      );
    }
    ...
    const ourMagiHash = ourMagi
      ? `@${ourMagi.data.energy}[${ourMagi.data.actionsUsed.map((action: string) => this.convertHash(action)).join(",")}]`
      : "X";
    ...
    return (
      (sim.state.activePlayer === sim.players[0] ? "*" : "v") +
      sim.state.step?.toString() +
      "{" +
      handHashes.join(",") +
      "}" +
      ourMagiHash +
      ...
      enemyMagiHash +
      (sim.state.prompt
        ? "?" + this.convertHash(sim.state.promptGeneratedBy || "")
        : "")
    );
  }
