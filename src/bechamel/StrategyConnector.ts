...
export class StrategyConnector {
  ...
  public connect(strategy: Strategy) {
    ...
    this.io.on(
      "gameData",
      (data: { playerId: number; state: SerializedClientState }) => {
        ...
        if (
          this.gameState.playerPriority(this.playerId) ||
    ...
    this.io.on(
      "action",
      (action: ClientAction | { type: "display/priority"; player: number }) => {
        if (this.gameState && this.playerId && action) {
          ...
          */
          ...
          if (this.gameState.hasGameEnded()) {
            console.log(
              this.gameState.getWinner() === this.playerId
                ? "We won"
                : "We lost",
            );
            this.io.close();
          ...
          if (action.type === "display/status") {
            ...
            console.log(
              `Prompt state: ${this.gameState.isInPromptState(this.playerId) ? "yes" : "no"}`,
            );
            if (this.gameState.isInPromptState(this.playerId)) {
              console.log(
                `Prompt type: ${this.gameState.getPromptType()}, player: ${this.gameState.state.promptPlayer}`,
              );
            } /* else {
  ...
  private requestAndSendAction() {
    ...
    if (
      ...
      if (currentStep !== 5) {
        ...
        console.log(
          `We are here in step ${currentStep}, and cannot request action`,
        );
      }
    }
  }
