  ...
  switch (selector) {
    case SELECTOR_OWN_CREATURES: {
      return (
        ...
        card.data.controller === source.data.controller
      );
    }
            ...
            case PROPERTY_POWER_COST: {
              if ("powers" in result.modifiedData) {
                result.modifiedData.powers = result.modifiedData.powers.map(
                  (power) => ({ ...power, cost: modifierFunction(power.cost) }),
                );
              }
              break;
            }
...
export function mapCardDataFromProps(state, { id }) {
  ...
  const foundZone = Object.values(state.zones).find((zone) =>
    zone.find(filter),
  );
  return {
...
type CardFilter = (card: ExpandedClientCard) => boolean;
    ...
    case PROMPT_TYPE_SINGLE_CREATURE_FILTERED:
      if (promptParams) {
        if (promptParams.restrictions && promptParams.restrictions.length) {
          const checkers = promptParams.restrictions.map(({ type, value }) =>
...
function getByProperty(target, property, subProperty = null) {
  switch (property) {
    ...
    case PROPERTY_POWER_COST:
      return target.card.data.powers.find(({ name }) => name === subProperty)
        .cost;
...
export const getCardDetails = (state: StateRepresentation) => {
  ...
  const allZonesCards = {
    inPlay: [...baseCards].map((card) => ({
      ...card,
      card: byName(card.card),
      originalCard: byName(card.card),
    })),
  ...
  const continuousStaticAbilities: StaticAbilityType[] = state.continuousEffects
    .map((effect) => effect.staticAbilities)
    .flat();
  const zoneAbilities: StaticAbilityExpanded[] = [
    ...allZonesCards.inPlay,
    ...allZonesCards.playerActiveMagi,
    ...allZonesCards.opponentActiveMagi,
  ].reduce(
    (acc, cardInPlay) =>
      cardInPlay.card.data.staticAbilities
        ? [
            ...acc,
            ...cardInPlay.card.data.staticAbilities.map((a) => ({
              ...a,
  ...
  ].sort((a, b) => propertyLayers[a.property] - propertyLayers[b.property]);
  ...
  const resultingZones = staticAbilities.reduce((oldState, staticAbility) => {
    ...
    for (let cardId in newState.inPlay) {
      ...
      if (
        cardMatchesSelector(currentCard, selector, selectorParameter, {
          data: { controller: staticAbility.player || null },
        })
      ) {
