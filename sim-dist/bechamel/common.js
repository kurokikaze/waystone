...
import { TYPE_CREATURE, TYPE_RELIC, TYPE_MAGI, PROMPT_TYPE_RELIC, PROMPT_TYPE_SINGLE_CREATURE, PROMPT_TYPE_SINGLE_CREATURE_OR_MAGI, PROMPT_TYPE_OWN_SINGLE_CREATURE, PROMPT_TYPE_ANY_CREATURE_EXCEPT_SOURCE, PROMPT_TYPE_SINGLE_CREATURE_FILTERED, RESTRICTION_TYPE, RESTRICTION_ENERGY_LESS_THAN_STARTING, RESTRICTION_ENERGY_LESS_THAN, RESTRICTION_OWN_CREATURE, RESTRICTION_OPPONENT_CREATURE, RESTRICTION_REGION, RESTRICTION_CREATURE_TYPE, RESTRICTION_ENERGY_EQUALS, RESTRICTION_CREATURE_WAS_ATTACKED, SELECTOR_OWN_CREATURES, SELECTOR_OWN_MAGI, SELECTOR_STATUS, SELECTOR_ID, SELECTOR_OWN_CREATURES_WITH_STATUS, SELECTOR_CREATURES_OF_PLAYER, CALCULATION_SET, CALCULATION_ADD, CALCULATION_SUBTRACT, CALCULATION_SUBTRACT_TO_MINIMUM_OF_ONE, CALCULATION_DOUBLE, CALCULATION_HALVE_ROUND_DOWN, CALCULATION_HALVE_ROUND_UP, CALCULATION_MIN, CALCULATION_MAX, PROPERTY_ID, PROPERTY_POWER_COST, PROPERTY_TYPE, PROPERTY_CREATURE_TYPES, PROPERTY_MAGI_NAME, PROPERTY_ENERGY_COUNT, PROPERTY_REGION, PROPERTY_CAN_BE_ATTACKED, PROPERTY_MAGI_STARTING_ENERGY, PROPERTY_STATUS_DEFEATED_CREATURE, PROPERTY_STATUS_WAS_ATTACKED, PROPERTY_ATTACKS_PER_TURN, PROPERTY_ENERGIZE, PROPERTY_CONTROLLER, PROPERTY_COST, PROPERTY_STATUS, PROPERTY_CAN_ATTACK_MAGI_DIRECTLY, PROPERTY_ABLE_TO_ATTACK, PROPERTY_ENERGY_LOSS_THRESHOLD, STATUS_BURROWED, } from 'moonlands/dist/esm/const.js';
        ...
        case PROMPT_TYPE_SINGLE_CREATURE_FILTERED:
            if (promptParams) {
                if (promptParams.restrictions && promptParams.restrictions.length) {
                    ...
                    return card => checkers.map(checker => checker(card)).every(a => a === true); // combine checkers
                }
                else {
...
export const getCardDetails = (state) => {
    ...
    const zoneAbilities = [...allZonesCards.inPlay, ...allZonesCards.playerActiveMagi, ...allZonesCards.opponentActiveMagi].reduce((acc, cardInPlay) => cardInPlay.card.data.staticAbilities ? [
        ...acc,
    ...
    const resultingZones = staticAbilities.reduce((oldState, staticAbility) => {
        const { name, text, selector, selectorParameter, property, subProperty, modifier } = staticAbility;
