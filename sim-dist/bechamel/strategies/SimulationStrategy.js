...
import { PROMPT_TYPE_MAY_ABILITY, ACTION_ATTACK, ACTION_PASS, ACTION_PLAY, ACTION_POWER, ACTION_RESOLVE_PROMPT, TYPE_CREATURE, TYPE_RELIC, PROMPT_TYPE_CHOOSE_UP_TO_N_CARDS_FROM_ZONE, } from "../const.js";
import { createState, getStateScore } from './simulationUtils.js';
import { HashBuilder } from './HashBuilder.js';
import { ActionExtractor } from './ActionExtractor.js';
import { PROMPT_TYPE_CHOOSE_N_CARDS_FROM_ZONE, PROMPT_TYPE_PAYMENT_SOURCE, ZONE_TYPE_IN_PLAY } from 'moonlands/dist/esm/const.js';
import { SimulationQueue } from './SimulationQueue.js';
const STEP_NAME = {
    ENERGIZE: 0,
    PRS1: 1,
    ATTACK: 2,
...
export class SimulationStrategy {
    ...
    static deckId = '5f60e45e11283f7c98d9259c'; // Local deck (Arderial)
    ...
    resolveTargetPrompt(target, targetName) {
        return {
    ...
    resolveCardsPrompt(cards, type, zone, zoneOwner) {
        return {
            type: ACTION_RESOLVE_PROMPT,
            promptType: type || this.gameState?.getPromptType(),
            zone,
    ...
    fixTargetPromptResolution(action) {
    ...
    fixCardPromptResolution(action) {
        const promptAvailableCards = this.gameState?.state.promptParams.cards?.map(({ id }) => id);
        // @ts-ignore
        if ('cards' in action && action.cards && action.cards instanceof Array && action.cards.some((card) => !promptAvailableCards?.includes(card))) {
            const availableCardPairs = {};
    ...
    generateAction() {
        ...
        if (this.actionsOnHold.length) {
            ...
            if (action.type == ACTION_RESOLVE_PROMPT &&
                ...
                return this.fixTargetPromptResolution(action);
            }
            return action;
        }
            ...
            if (this.gameState.playerPriority(this.playerId)) {
                ...
                switch (step) {
                    ...
                    case STEP_NAME.PRS2: {
                        ...
                        this.graph = '';
                        const outerSim = createState(this.gameState, this.playerId || 2, TEMPORARY_OPPONENT_ID);
                        ...
                        this.actionsOnHold = bestActions.slice(1).filter(({ action }) => (action.type === ACTION_PLAY && 'payload' in action && action.payload.player === this.playerId) ||
                            ('player' in action && action.player === this.playerId)).map(({ action, hash }) => ({
    ...
    resolveChooseCardsPrompt() {
        ...
        return this.resolveCardsPrompt(availableCards.slice(0, this.gameState.state.promptParams.numberOfCards || 0)
            .map(card => card), this.gameState.state.promptType || '', this.gameState.state.promptParams.zone || ZONE_TYPE_IN_PLAY, this.gameState.state.promptParams.zoneOwner || 0);
    }
}
