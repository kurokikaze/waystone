import { ZONE_TYPE_ACTIVE_MAGI, ZONE_TYPE_MAGI_PILE, ZONE_TYPE_DEFEATED_MAGI, ZONE_TYPE_DECK, ZONE_TYPE_DISCARD, ZONE_TYPE_HAND, ZONE_TYPE_IN_PLAY, EXPIRATION_NEVER, EXPIRATION_ANY_TURNS, EXPIRATION_OPPONENT_TURNS, } from 'moonlands/src/const';
...
export const getZoneName = (serverZoneType, source, playerId) => {
...
export const tickDownContinuousEffects = (effects, opponent) => {
...
export const cleanupContinuousEffects = (effects, opponent) => {
    let resultingEffects = [];
    for (const effect of effects) {
        switch (effect.expiration.type) {
            case EXPIRATION_NEVER: {
...
export const affectRemoveEnergy = (state, card, action) => {
...
export const affectAddEnergy = (state, card, action) => {
    const gameCard = byName(card.card);
    const source = action.source ? findInPlay(state, action.source.id) : null;
    if (gameCard && gameCard.data.energyStasis && source && source.data.controller === card.data.controller) {
        // For the Colossus
